...
Specs (initiatives, features, slices, tasks) for this repo all live as GitHub issues on `tylerdurrett/app-starter`. Use the `gh` CLI for all operations. `gh` resolves the repo automatically from `git remote -v` (the `origin` remote points at `tylerdurrett/app-starter`).
...
```bash
gh issue list --search "parent-issue:tylerdurrett/app-starter#59"
```
