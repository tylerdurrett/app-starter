...
beforeAll(async () => {
  ...
  const ts = Date.now();
  ({ userId: ownerId } = await signUp(app, `resolver-owner-${ts}@test.com`, 'Owner'));
  ({ userId: wsOwnerId } = await signUp(app, `resolver-wsowner-${ts}@test.com`, 'WsOwner'));
  ({ userId: wsManagerId } = await signUp(app, `resolver-wsmgr-${ts}@test.com`, 'WsMgr'));
  ({ userId: wsMemberId } = await signUp(app, `resolver-wsmem-${ts}@test.com`, 'WsMem'));
  ({ userId: outsiderId } = await signUp(app, `resolver-outsider-${ts}@test.com`, 'Outsider'));
});
...
describe('getAuthorizedProjectBySlug', () => {
  it('returns the canonical project projection for a direct project owner', async () => {
    const ws = await createWorkspaceViaService('Canonical Direct Owner Ws', wsOwnerId);
    const proj = await createProjectViaService('Canonical Direct Owner Proj', ws.id, ownerId);
  ...
  it.each(['manager', 'member'] as const)('returns a direct project %s role', async (role) => {
    const ws = await createWorkspaceViaService(`Canonical Direct ${role} Ws`, wsOwnerId);
    const proj = await createProjectViaService(`Canonical Direct ${role} Proj`, ws.id, ownerId);
  ...
  it.each([
    ...
    async (workspaceRole, expectedProjectRole) => {
      const ws = await createWorkspaceViaService(
        `Canonical Override ${workspaceRole} Ws`,
        wsOwnerId,
      );
      const proj = await createProjectViaService(
        `Canonical Override ${workspaceRole} Proj`,
        ws.id,
        ownerId,
      );
  ...
  it('prefers a weaker direct role over a stronger workspace override', async () => {
    const ws = await createWorkspaceViaService('Canonical Precedence Ws', wsOwnerId);
    const proj = await createProjectViaService('Canonical Precedence Proj', ws.id, ownerId);
  ...
  it('authorizes direct project access without workspace membership', async () => {
    const ws = await createWorkspaceViaService('Canonical Direct Only Ws', wsOwnerId);
    const proj = await createProjectViaService('Canonical Direct Only Proj', ws.id, ownerId);
  ...
  it('resolves duplicate project slugs by workspace', async () => {
    const wsA = await createWorkspaceViaService('Canonical Duplicate Ws A', wsOwnerId);
    const wsB = await createWorkspaceViaService('Canonical Duplicate Ws B', wsOwnerId);
    const projA = await createProjectViaService('Canonical Shared Proj', wsA.id, ownerId);
    const projB = await createProjectViaService('Canonical Shared Proj', wsB.id, ownerId);
  ...
  it('returns NOT_FOUND when the project belongs to a different workspace', async () => {
    const wsA = await createWorkspaceViaService('Canonical Wrong Workspace A', wsOwnerId);
    const wsB = await createWorkspaceViaService('Canonical Wrong Workspace B', wsOwnerId);
    const proj = await createProjectViaService('Canonical Wrong Workspace Proj', wsA.id, ownerId);

    await expect(getAuthorizedProjectBySlug(wsB.slug, proj.slug, ownerId)).rejects.toMatchObject({
      code: 'NOT_FOUND',
    });
  });

  it('returns NOT_FOUND for a missing workspace or project', async () => {
    const ws = await createWorkspaceViaService('Canonical Missing Ws', wsOwnerId);
  ...
  it('returns NOT_FOUND when the actor has no project or workspace access', async () => {
    const ws = await createWorkspaceViaService('Canonical Inaccessible Ws', wsOwnerId);
    const proj = await createProjectViaService('Canonical Inaccessible Proj', ws.id, ownerId);

    await expect(getAuthorizedProjectBySlug(ws.slug, proj.slug, outsiderId)).rejects.toMatchObject({
      code: 'NOT_FOUND',
    });
  });
});
...
describe('findAuthorizedProjectById', () => {
  it('returns the exact canonical projection for a direct owner', async () => {
    ...
    const ws = await createWorkspaceViaService('Canonical Find Direct Owner Ws', wsOwnerId);
    const proj = await createProjectViaService('Canonical Find Direct Owner Proj', ws.id, actorId);
  ...
  it.each(['manager', 'member'] as const)('returns a direct %s role', async (role) => {
    ...
    const ws = await createWorkspaceViaService(`Canonical Find Direct ${role} Ws`, wsOwnerId);
    const proj = await createProjectViaService(
      `Canonical Find Direct ${role} Proj`,
      ws.id,
      ownerId,
    );
  ...
  it('prefers a weaker direct role over workspace-owner access', async () => {
    ...
    const ws = await createWorkspaceViaService('Canonical Find Precedence Ws', actorId);
    const proj = await createProjectViaService('Canonical Find Precedence Proj', ws.id, ownerId);
  ...
  it('falls back to workspace access after direct access is removed, then returns null', async () => {
    ...
    const ws = await createWorkspaceViaService('Canonical Find Access Loss Ws', wsOwnerId);
    const proj = await createProjectViaService('Canonical Find Access Loss Proj', ws.id, ownerId);
    ...
    await db
      ...
      .where(
        and(eq(projectMemberships.projectId, proj.id), eq(projectMemberships.userId, actorId)),
      );
    await expect(findAuthorizedProjectById(proj.id, actorId)).resolves.toMatchObject({
      role: 'owner',
    });
    ...
    await db
      ...
      .where(
        and(eq(workspaceMemberships.workspaceId, ws.id), eq(workspaceMemberships.userId, actorId)),
      );
  ...
  it('returns null without throwing for missing and inaccessible projects', async () => {
    ...
    const ws = await createWorkspaceViaService('Canonical Find Inaccessible Ws', wsOwnerId);
    const proj = await createProjectViaService('Canonical Find Inaccessible Proj', ws.id, ownerId);
...
describe('listAuthorizedProjectsForUser', () => {
  it('returns the exact flat projection in global project-created order', async () => {
    ...
    const wsA = await createWorkspaceViaService('Canonical List Exact Ws A', wsOwnerId);
    const wsB = await createWorkspaceViaService('Canonical List Exact Ws B', wsOwnerId);
    const projA = await createProjectViaService('Canonical List Exact Proj A', wsA.id, ownerId);
    const projB = await createProjectViaService('Canonical List Exact Proj B', wsB.id, ownerId);
  ...
  it.each([
    ...
    async (workspaceRole, expectedProjectRole) => {
      ...
      const projA = await createProjectViaService(
        `Canonical List ${workspaceRole} Proj A`,
        ws.id,
        ownerId,
      );
      const projB = await createProjectViaService(
        `Canonical List ${workspaceRole} Proj B`,
        ws.id,
        ownerId,
      );
  ...
  it('filters direct-only access without requiring workspace membership or leaking siblings', async () => {
    ...
    const wsA = await createWorkspaceViaService('Canonical List Direct Filter Ws A', wsOwnerId);
    const wsB = await createWorkspaceViaService('Canonical List Direct Filter Ws B', wsOwnerId);
    const projA = await createProjectViaService(
      'Canonical List Direct Filter Proj A',
      wsA.id,
      ownerId,
    );
    const siblingA = await createProjectViaService(
      'Canonical List Direct Filter Sibling A',
      wsA.id,
      ownerId,
    );
    const projB = await createProjectViaService(
      'Canonical List Direct Filter Proj B',
      wsB.id,
      ownerId,
    );
  ...
  it('deduplicates projects and gives a weaker direct membership precedence', async () => {
    ...
    const ws = await createWorkspaceViaService('Canonical List Precedence Ws', actorId);
    const proj = await createProjectViaService('Canonical List Precedence Proj', ws.id, ownerId);
    await addProjectMember(proj.id, actorId, 'member');
  ...
  it('combines access across workspaces while omitting inaccessible projects', async () => {
    ...
    const directWs = await createWorkspaceViaService('Canonical List Mixed Direct Ws', wsOwnerId);
    const inheritedWs = await createWorkspaceViaService(
      'Canonical List Mixed Inherited Ws',
      wsOwnerId,
    );
    const inaccessibleWs = await createWorkspaceViaService(
      'Canonical List Mixed Inaccessible Ws',
      wsOwnerId,
    );
    const directProj = await createProjectViaService(
      'Canonical List Mixed Direct Proj',
      directWs.id,
      ownerId,
    );
    const inheritedA = await createProjectViaService(
      'Canonical List Mixed Inherited A',
      inheritedWs.id,
      ownerId,
    );
    const inheritedB = await createProjectViaService(
      'Canonical List Mixed Inherited B',
      inheritedWs.id,
      ownerId,
    );
    const inaccessibleProj = await createProjectViaService(
  ...
  it('returns an empty list for unknown and wholly inaccessible workspace filters', async () => {
    ...
    const ws = await createWorkspaceViaService('Canonical List Empty Filter Ws', wsOwnerId);
    await createProjectViaService('Canonical List Empty Filter Proj', ws.id, ownerId);
...
describe('resolveProjectWithOverride', () => {
  it('throws NOT_FOUND for a non-existent project slug', async () => {
    const ws = await createWorkspaceViaService('Nonexistent Slug Ws', wsOwnerId);
    await expect(
  ...
  it('returns direct project member role with no workspace override flag', async () => {
    const ws = await createWorkspaceViaService('Direct Member Ws', wsOwnerId);
    const proj = await createProjectViaService('Direct Member Proj', ws.id, ownerId);
  ...
  it('returns "manager" role for a direct project manager', async () => {
    const ws = await createWorkspaceViaService('Direct Mgr Ws', wsOwnerId);
    const proj = await createProjectViaService('Direct Mgr Proj', ws.id, ownerId);
  ...
  it('returns "member" role for a direct project member', async () => {
    const ws = await createWorkspaceViaService('Direct Mem Ws', wsOwnerId);
    const proj = await createProjectViaService('Direct Mem Proj', ws.id, ownerId);
  ...
  it('workspace owner without project membership gets synthetic owner role + override flag', async () => {
    const ws = await createWorkspaceViaService('WsOwner Override Ws', wsOwnerId);
    // Project created by ownerId; wsOwner has no direct project membership.
    const proj = await createProjectViaService('WsOwner Override Proj', ws.id, ownerId);
  ...
  it('workspace manager without project membership gets synthetic owner role + override flag', async () => {
    const ws = await createWorkspaceViaService('WsMgr Override Ws', wsOwnerId);
    await addWorkspaceMember(ws.id, wsManagerId, 'manager');
    const proj = await createProjectViaService('WsMgr Override Proj', ws.id, ownerId);
  ...
  it('workspace member without project membership gets synthetic member read access', async () => {
    const ws = await createWorkspaceViaService('WsMem Read Access Ws', wsOwnerId);
    await addWorkspaceMember(ws.id, wsMemberId, 'member');
    const proj = await createProjectViaService('WsMem Read Access Proj', ws.id, ownerId);
  ...
  it('workspace member without project membership cannot edit the project', async () => {
    const ws = await createWorkspaceViaService('WsMem Edit Access Ws', wsOwnerId);
    await addWorkspaceMember(ws.id, wsMemberId, 'member');
    const proj = await createProjectViaService('WsMem Edit Access Proj', ws.id, ownerId);
  ...
  it('outsider (no workspace or project access) gets NOT_FOUND', async () => {
    const ws = await createWorkspaceViaService('Outsider Ws', wsOwnerId);
    const proj = await createProjectViaService('Outsider Proj', ws.id, ownerId);
  ...
  it('direct project membership takes precedence over workspace override', async () => {
    const ws = await createWorkspaceViaService('Precedence Ws', wsOwnerId);
    ...
    const proj = await createProjectViaService('Precedence Proj', ws.id, ownerId);
    await addProjectMember(proj.id, wsOwnerId, 'member');
  ...
  it('applies requiredPermission and throws FORBIDDEN when denied', async () => {
    const ws = await createWorkspaceViaService('Perm Ws', wsOwnerId);
    const proj = await createProjectViaService('Perm Proj', ws.id, ownerId);
    await addProjectMember(proj.id, wsMemberId, 'member');
  ...
  it('workspace admin override satisfies project:delete via synthetic owner role', async () => {
    const ws = await createWorkspaceViaService('Perm Override Ws', wsOwnerId);
    const proj = await createProjectViaService('Perm Override Proj', ws.id, ownerId);
...
describe('resolveProjectWithOverride by (workspace, slug)', () => {
  it('resolves each duplicate-slug project under its own workspace', async () => {
    const wsA = await createWorkspaceViaService('Composite Ws A', wsOwnerId);
    const wsB = await createWorkspaceViaService('Composite Ws B', wsOwnerId);
    // Same name in both workspaces -> identical bare slug (slug is workspace-scoped).
    const projA = await createProjectViaService('Shared Slug Proj', wsA.id, ownerId);
    const projB = await createProjectViaService('Shared Slug Proj', wsB.id, ownerId);
    expect(projA.slug).toBe(projB.slug);
  ...
  it('resolves via composite key without requiring workspace membership (non-authorizing join)', async () => {
    ...
    const ws = await createWorkspaceViaService('Composite NonMember Ws', wsOwnerId);
    const proj = await createProjectViaService('Composite NonMember Proj', ws.id, ownerId);
  ...
  it('returns NOT_FOUND when the slug exists only in a different workspace', async () => {
    const wsA = await createWorkspaceViaService('Wrong Ws Source', wsOwnerId);
    const wsB = await createWorkspaceViaService('Wrong Ws Target', wsOwnerId);
    const proj = await createProjectViaService('Only In A Proj', wsA.id, ownerId);
  ...
  it('returns NOT_FOUND for an unknown workspace slug', async () => {
    const ws = await createWorkspaceViaService('Composite Unknown Ws', wsOwnerId);
    const proj = await createProjectViaService('Composite Unknown Proj', ws.id, ownerId);
  ...
  it('preserves NOT_FOUND (not FORBIDDEN) access denial under the composite lookup', async () => {
    ...
    const ws = await createWorkspaceViaService('Composite Denial Ws', wsOwnerId);
    const proj = await createProjectViaService('Composite Denial Proj', ws.id, ownerId);
