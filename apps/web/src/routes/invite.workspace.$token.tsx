...
export const Route = createFileRoute('/invite/workspace/$token')({
  loader: async ({ params }): Promise<WorkspaceInviteMetadata | null> => {
    try {
      return await getWorkspaceInviteByToken(params.token);
    } catch (err) {
  ...
  if (invite.status === 'revoked') {
    return (
      <TerminalCard
        ...
        message={<>This invite to <strong>{invite.workspaceName}</strong> has been revoked by the workspace owner.</>}
      />
    );
  }
  ...
  if (invite.status === 'accepted') {
    return (
      <TerminalCard
        ...
        message={<>This invite to <strong>{invite.workspaceName}</strong> has already been accepted.</>}
        linkTo="/w/$workspaceSlug"
        linkParams={{ workspaceSlug: invite.workspaceSlug }}
  ...
  if (new Date(invite.expiresAt) < new Date()) {
    return (
      <TerminalCard
        ...
        message={<>This invite to <strong>{invite.workspaceName}</strong> has expired. Please ask the workspace owner to send a new invite.</>}
      />
    );
  }
...
function PendingInviteView({ invite }: { invite: WorkspaceInviteMetadata }) {
  ...
  const handleAccept = async () => {
    ...
    try {
      const result = await acceptWorkspaceInviteByToken(token);
      await navigate({ to: '/w/$workspaceSlug', params: { workspaceSlug: result.workspaceSlug } });
    } catch (err) {
      ...
      <CardContent className="space-y-6">
        ...
        {!isLoggedIn && (
          <div className="space-y-3">
            <Link to="/login" search={{ redirectTo: `/invite/workspace/${token}` }}>
              <Button className="w-full">Sign in to accept</Button>
            </Link>
            <Link to="/register" search={{ redirectTo: `/invite/workspace/${token}` }}>
